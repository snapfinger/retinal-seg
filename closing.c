/*
implement the closing (first dilation, then erosion) for gray-scale image
*/
...
Vfembed(&tm1,&im,0,0,0,0); //for max
...
for(y=im.ylo;y<=im.yhi;y++){
	...
	for(x=im.xlo;x<=im.xhi;x++){
		...
		for(u=y0;u<=y1;u++){
			for(v=x0;v<=x1;v++){
				if((y-u)*(y-u)+(x-v)*(x-v)<=radius*radius && im.u[u][v]>maxVal){
					maxVal=im.u[u][v];
				}
			}
		}
...
for(y=tm1.ylo;y<=tm1.yhi;y++){
	...
	for(x=tm1.xlo;x<=tm1.xhi;x++){
		...
		int minVal=tm1.u[y][x];
		
		int u,v;
		for(u=y0;u<=x1;u++){
			for(v=y0;v<=y1;v++){
				if((y-u)*(y-u)+(x-v)*(x-v)<=radius*radius && tm1.u[u][v]<minVal){
					minVal=tm1.u[u][v];
				}
			}
		}
...
fprintf(stderr,"closing completed\n");
